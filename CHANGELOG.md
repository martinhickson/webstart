...
- Added jardiff-path variants (direct source access blocked, javaws User-Agent, suffix range ignored on the jardiff response, sequential requests, `If-Modified-Since` on the source), malformed and wildcard q-values, pack200 q-zero on versioned resources, concurrent `If-Range` match/stale mix, and wildcard version matching on `version.xml`-mapped resources
- Cached jardiffs are now invalidated when either source jar changes (e.g. after a redeploy): `JarDiffEntry` records the source files' identity and a stale entry is regenerated instead of serving an outdated patch
